...
class UsuariosController extends Controller
{
    ...
    //CORREGIDO PARA QUE NO EXCLUYA A LOS BENEFICIARIOS SIN REGISTRO SOCIAL
    public function ajaxbeneficiarios()
    {
        $beneficiarios = Beneficiario::with('registrosocial')->get();

        $rows = [];
        foreach ($beneficiarios as $b) {
            // Solo excluir los marcados explícitamente como fallecido 'F'
            if (isset($b->fallecido) && $b->fallecido === 'F') {
                continue;
            }

            $reg = $b->registrosocial;

            $rows[] = [
                'id' => $b->id,
                'rut' => $b->rut,
                'nombre' => trim(($b->nombres ?? '') . ' ' . ($b->apellidos ?? '')) . ' (' . ($b->fecha_nacimiento ? Carbon::parse($b->fecha_nacimiento)->age : 'N/A') . ' Años)',
                'registrosocial' => $reg ? (($reg->folioid ?? 'Sin folio') . ' (' . ($reg->porcentaje ?? 0) . '% Fecha: ' . ($reg->updated_at ? $reg->updated_at->format('d-m-Y') : ($reg->fechainforme ?? 'Sin fecha')) . ')') : 'Sin registro',
                'direccion' => trim(($b->direccion ?? '') . ($b->sector ? ' ,' . $b->sector : '')),
                'comentario' => $b->comentario ?? '',
                'idficha' => $reg->id ?? null,
                'porcentaje' => $reg->porcentaje ?? null,
            ];
        }

        return DataTables($rows)->toJson();
    }

    //AGREGADO 
    public function registrosocial()
    {
        return $this->belongsTo(RegistroSocial::class, 'rsh_id');
    }
    ...
    public function store(Request $request)
    {
        ...
        $beneficiario->grupofamiliar = $request->grupofamiliar;
    ...
    public function solicitar(string $id)
    {
        ...
        if (Auth::user()->name == 'Alejandra Rojas') {

            return view('solicitudes.solicitar-materialconstr', ['materiales' => $materiales, 'beneficiario' => $beneficiario]);
        } else {
    ...
    public function solicitudparte1(string $id, Request $request)
    {
        ...
        foreach ($request->materiales as $key => $m) {
            ...
            if (is_null($request->emergencia)) {
                ...
                $seleccionados[$key]['id'] = $material->id;
                $seleccionados[$key]['nombre'] = $material->nombre;
                $seleccionados[$key]['limite'] = $material->limite_urgencia;
                $seleccionados[$key]['medida'] = $material->medida;
    ...
    public function storesituacion(Request $request, $id)
    {
        ...
        $situacion->asistente = auth()->user()->name;
